...
def start_server() -> subprocess.Popen[str]:
    ...
    auth_adapter = str(project_root / "scripts" / "gcloud_adc")
    env["PYTHONPATH"] = os.pathsep.join(
        item for item in (auth_adapter, env.get("PYTHONPATH", "")) if item
    )
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    data = {
        ...
        "new_message": {
            "role": "user",
            "parts": [{"text": "Review demo case cedar-senior."}],
        },
        "streaming": True,
    }
...
def test_a2a_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    message = Message(
        ...
        role=Role.user,
        parts=[Part(root=TextPart(text="Review demo case cedar-senior."))],
    )
